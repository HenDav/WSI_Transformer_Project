...
class Mag_Dataset(Dataset):
    ...
    def __init__(self, train: bool = True, transform_type: str = 'flip', tile_size = 256, color_param: float = 0.1,):
        self.pictures #array that saves all the images 
...
class WSI_Master_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        if self.DataSet == 'CAT' or self.DataSet == 'ABCTB_TCGA' or self.DataSet == 'TCGA':
        ...
        if self.HL >= 0:
            ...
            if self.HL == 0:
                ...
                col_name = f'label_HL_q{self.HL}'
            target_df = pd.read_excel('./Summary_data_per_slide_with_HL_other_quantiles.xlsx')
            target_df = target_df [target_df[col_name].notna()]
            print(f'len of hl df is {len(target_df)}')
            counter = 0
        elif self.ESR >= 0:
            new_indices = []
            col_name = f'label_VANT_ESR1_q{self.ESR}'
            target_df = pd.read_excel('./Summary_data_per_slide_with_ESR1.xlsx')
            target_df = target_df [target_df[col_name].notna()]
            print(f'len of esr df is {len(target_df)}')
            counter = 0
        elif self.ERBB >= 0:
            new_indices = []
            col_name = f'label_ERBB2_q{self.ERBB}'
            target_df = pd.read_excel('./Summary_data_per_slide_with_ERBB2.xlsx')
            target_df = target_df [target_df[col_name].notna()]
            print(f'len of erbb df is {len(target_df)}')
            counter = 0
        ...
        for _, index in enumerate(tqdm(valid_slide_indices)):
            if (self.DX and all_is_DX_cut[index]) or not self.DX:
                if self.HL >= 0 or self.ESR >= 0 or self.ERBB >= 0:
                    ...
                    slides = target_df[target_df['DatasetName'] == dataset]
                ...
                if self.HL < 0 and self.ESR < 0 and self.ERBB < 0:
        ...
        if self.HL >= 0 or self.ESR >= 0 or self.ERBB >= 0:
            self.valid_slide_indices = new_indices
            print(f'skipped {counter} slides because of no target label')
    ...
    def __init__(
        ...
        if sys.platform == 'linux':
            if dataset in ['TA 6', 'TCGA_ABCTB']:
                ...
                grid_location_dict = {
                    'TCGA_GRADE':
                    path + r'Breast/TCGA/Grids_10/Grid_data.xlsx'
                }
                slide_data_DF_dict = {
                    'TCGA_GRADE':
                    pd.read_excel(
                        path + 'Breast/TCGA/slides_data_TCGA_GRADE.xlsx'
                    )
                }
                ...
                grid_location_dict = {
                    f'FINHER{i}':
                    path + fr'Breast/FinHer/Batch{i}/FINHER_{i}/Grids_10/Grid_data.xlsx'
                    for i in range(1, 4)
                }
                slide_data_DF_dict = {
                    f'FINHER{i}': pd.read_excel(
                        path + fr'Breast/FinHer/Batch{i}/FINHER_{i}/slides_data_FINHER_{i}.xlsx'
                    )
                    for i in range(1, 4)
                }
                ...
                grid_location_dict = {
                    f'SHEBA{i}':
                    path + fr'Breast/Sheba/Batch_{i}/SHEBA{i}/Grids_10/Grid_data.xlsx'
                    for i in range(2, 7)
                }
                slide_data_DF_dict = {
                    f'SHEBA{i}': pd.read_excel(
                        path + fr'Breast/Sheba/Batch_{i}/SHEBA{i}/slides_data_SHEBA{i}.xlsx'
                    )
                    for i in range(2, 7)
                }
            ...
            elif dataset == "TAILORx_1":

                assert name_of_gipdeep_host_node() == "gipdeep10", "TAILORx_1 only usable on gipdeep10 node"
                
                path = '/SSDStorage/'

                grid_location_dict = {
                    'TAILORx_1':
                    path + r'Breast/TAILORx/Deid/TAILORx_1/Grids_10/Grid_data.xlsx'
                }
                slide_data_DF_dict = {
                    'TAILORx_1':
                    pd.read_excel(
                        path + r'Breast/TAILORx/Deid/TAILORx_1/slides_data_TAILORx_1.xlsx'
                    )
                }
            
            elif dataset == "EGA":

                assert name_of_gipdeep_host_node() == "gipdeep10", "EGA only usable on gipdeep10 node"
                
                path = '/SSDStorage/'

                grid_location_dict = {
                    'EGA_0':
                    path + r'Breast/EGA/Batch_1/EGA_0/Grids_10/Grid_data.xlsx'
                }
                slide_data_DF_dict = {
                    'EGA_0':
                    pd.read_excel(
                        path + r'Breast/EGA/Batch_1/EGA_0/slides_data_EGA_0.xlsx'
                    )
                }
        ...
        for file_idx, file in enumerate(tqdm(data_files)):
            ...
            for slide_num in range(num_slides):
                ...
                if len(dataset.split('_')) > 1 and dataset.split(
                        '_')[-1].isnumeric() and dataset != 'TAILORx_1':
