...
def get_datasets_dir_dict(Dataset: str):
    ...
    SHEBA_ssd_path = ssd_path + r'Breast/Sheba'
    ...
    FINHER_ssd_path = ssd_path + r'Breast/FinHer/'
    ...
    TailorX_gipdeep10_path = ssd_path + r'Breast/TAILORx/Deid/TAILORx_1/'
    EGA_gipdeep10_path = ssd_path + r'Breast/EGA/Batch_1/EGA_0/'
    ...
    if Dataset == 'ABCTB_TCGA':
        ...
        finher_path = FINHER_gipdeep_path if not gipdeep10_used else FINHER_ssd_path
        if sys.platform == 'linux':  # GIPdeep
            for ii in np.arange(1, 4):
                dir_dict['FINHER_' + str(ii)] = os.path.join(finher_path, 'Batch' + str(ii), 'FINHER_' + str(ii))
                
    elif Dataset == 'FINHER_2Folds':
        finher_path = FINHER_gipdeep_path if not gipdeep10_used else FINHER_ssd_path
        if sys.platform == 'linux':  # GIPdeep
            for ii in np.arange(1, 4):
                dir_dict['FINHER_2Folds_' + str(ii)] = os.path.join(finher_path, 'Batch' + str(ii), 'FINHER_' + str(ii))
    ...
    elif Dataset == 'TCGA_GRADE':
        if sys.platform == 'linux':  # GIPdeep
            tcga_path = TCGA_gipdeep_path if not gipdeep10_used else TCGA_ssd_path
            dir_dict['TCGA_GRADE'] = tcga_path
        ...
        if sys.platform == 'linux':
            for ii in np.arange(2, 7):
                sheba_path = SHEBA_gipdeep_path if not gipdeep10_used else SHEBA_ssd_path
                dir_dict['SHEBA' + str(ii)] = os.path.join(sheba_path, 'Batch_' + str(ii), 'SHEBA' + str(ii))
        elif sys.platform == 'win32':  # Ran local
            for ii in np.arange(2, 7):
        ...
        dir_dict["TAILORx_1"] = TailorX_gipdeep10_path
        
    elif Dataset == "TAILORx_2Folds":
        dir_dict["TAILORx_2Folds"] = TailorX_gipdeep10_path
        
    elif Dataset == "EGA":
        dir_dict["EGA_0"] = EGA_gipdeep10_path
