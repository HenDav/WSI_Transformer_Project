...
from Survival.Cox_Loss import Combined_loss
...
main_dir = 'Test_Run/Data_' + args.data_diff + '/Loss_Combination_Step_' + str(args.lr) + '_Censored_' + str(args.censored_ratio) + '_Weights_' + '_'.join([str(num) for num in args.loss_weights])
...
elif args.censored_ratio == 0:  # All sampled are NOT censored
    train_set = C_Index_Test_Dataset(train=True, data_difficulty=args.data_diff, is_all_not_censored=True)

elif args.censored_ratio > 0 and args.censored_ratio < 1:
    train_set_censored = C_Index_Test_Dataset(train=True, data_difficulty=args.data_diff, is_all_censored=True)
    train_set_not_censored = C_Index_Test_Dataset(train=True, data_difficulty=args.data_diff, is_all_not_censored=True)
