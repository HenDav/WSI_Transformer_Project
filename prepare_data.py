...
parser.add_argument('--tiles_to_extract', type=int, default=-1, help='Number of hard coded tiles to extract')
parser.add_argument('--resize_tiles_to', type=int, default=None, help='resize tiles to the specified size')
...
if __name__ =='__main__':
    ...
    if args.hard_copy:
        utils_data_managment.make_tiles_hard_copy(DataSet=args.dataset,
                                                  ROOT_DIR=args.data_root,
                                                  tile_sz=args.tile_size,
                                                  num_tiles=args.tiles_to_extract,
                                                  desired_magnification=args.mag,
                                                  added_extension=args.added_extension,
                                                  num_workers=num_workers,
                                                  oversized_HC_tiles=args.oversized_HC_tiles,
                                                  as_jpg=args.as_jpg,
                                                  resize_tiles_to_size=args.resize_tiles_to)
    if args.sl2im:
