...
def slide_2_image(slide_file, ind, mag, n_legit_tiles, desired_mag, grid_only):
    ...
    if not grid_only:
        ...
        n_patches = np.minimum(n_patches//2, n_legit_tiles)
        ...
        for ii in range(n_patches):
            grid[2*ii].imshow(tiles[ii])
            grid[2*ii].set_yticklabels([])
            grid[2*ii].set_xticklabels([])

            tile_rescale = tiles[ii].resize((patch_size//4, patch_size//4))
            tile_rescale2 = tile_rescale.resize((patch_size, patch_size))

            grid[2 * ii + 1].imshow(tile_rescale2)
            grid[2 * ii + 1].set_yticklabels([])
            grid[2 * ii + 1].set_xticklabels([])
