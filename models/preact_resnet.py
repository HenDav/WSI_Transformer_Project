...
class PreActBottleNeck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.residual = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels), nn.ReLU(inplace=True),
            nn.Conv2d(out_channels, out_channels * PreActBottleNeck.expansion,
                      1))
...
class PreActResNet_Ron(nn.Module):
    ...
    def forward(self, x):
        if len(x.shape) == 5:
            ...
            x = torch.reshape(
                x, (num_of_bags * tiles_amount, 3, tiles_size, tiles_size))

        if self.is_HeatMap:
            if self.training is True:
                raise Exception(
                    'Pay Attention that the model in not in eval mode')
            ...
            if x.shape[2] <= 256 or x.shape[2] == 1024 or x.shape[2] >= 2048:
                image_to_compute_MilWeights = F.avg_pool2d(
                    out,
                    kernel_size=32,
                    stride=1,
                    padding=16,
                ...
                vectored_image = torch.transpose(
                    torch.reshape(out.squeeze(0),
                                  (out.size(1), out.size(2) * out.size(3))), 1,
                    0)
                vectored_heat_image_2_channels = self.linear(vectored_image)
                vectored_heat_image = vectored_heat_image_2_channels[:,
                                                                     1] - vectored_heat_image_2_channels[:,
                                                                                                         0]
                small_heat_map = vectored_heat_image.view(
                    1, 1, initial_image_size, initial_image_size)

                # Upsampling the heat map:
                large_heat_map = F.interpolate(small_heat_map,
                                               size=x.shape[2],
                                               mode='bilinear')
                ...
                data_dict_4_gil = {
                    'linear_weights': self.linear.weight.cpu().numpy(),
                ...
                out_data_dict = {
                    ...
                    'Features':
                    features,
                    'Large Image for MIL Weights':
                    image_to_compute_MilWeights,
                    'Large Image for MIL Weights Without Averaging Sliding Window':
                    out_for_dict
                }
            ...
            if self.training and torch.any(torch.isnan(out)):
                ...
                scores_dict = {
                    'Scores':
                    list(torch.reshape(out, (18, )).detach().cpu().numpy())
                }
                scores_DF = pd.DataFrame(scores_dict).transpose()
                scores_DF.to_excel('debug_data_scores_from_model.xlsx')

                weights = list(
                    torch.reshape(self.linear.weight,
                                  (512, )).detach().cpu().numpy())
                bias = list(
                    self.linear.bias.detach().cpu().numpy()) + [-1] * 511

                linear_layer_dict = {'Weights': weights, 'Bias': bias}
...
class PreActResNet50(PreActResNet_Ron):
...
def PreActResNet50_Ron(train_classifier_only=False, num_classes=2):
    ...
    if train_classifier_only:
        if num_classes != 2:
            model.model_name = model.model_name[:-1] + ', num_classes=' + str(
                num_classes) + ')'
