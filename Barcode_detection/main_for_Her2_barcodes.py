...
parser.add_argument('-in', '--img_dir', type=str, default=r'C:\ran_data\RAMBAM\H24-27 taken', help='input directory')
...
def estimate_1d_grid_params_with_ransac(N_rows, upper_left_barcode_pos, ax):
    ...
    '''plt.figure()
    plt.plot(Nx[ransac.inlier_mask_], arr_for_ransac[ransac.inlier_mask_], '*g')
    plt.plot(Nx[~ransac.inlier_mask_], arr_for_ransac[~ransac.inlier_mask_], '*r')
    plt.plot(Nx, intercept + d * Nx, color="navy", linewidth=2)'''
    return intercept, d
...
def get_corrected_grid():
    ...
    return (new_x, new_y)
    # plt.plot(new_x, new_y, 'ob')
    '''for ii in range(N_codes):
        plt.text(x_arr[ii], y_arr[ii], str(ii), color='r')

    x_arr = np.array([code.rect[0] for code in codes_1d])
    y_arr = np.array([code.rect[1] for code in codes_1d])
    plt.plot(x_arr, y_arr, '*r')
    
    dy_estimate = (np.max(y_arr) - np.min(y_arr)) / N_rows
    dx_estimate = (np.max(x_arr) - np.min(x_arr)) / N_columns

    Ny = (y_arr - y_arr[0]) // dy_estimate
    Nx = (x_arr - x_arr[0]) // dx_estimate

    ransac = linear_model.RANSACRegressor()
    ransac.fit(Ny.reshape(-1, 1), y_arr)
    y0 = ransac.estimator_.intercept_
    dy = ransac.estimator_.coef_
    
    unit_grid_x, unit_grid_y = np.meshgrid(np.arange(12), np.arange(N_rows))
    unit_grid = np.column_stack((unit_grid_x.flatten(), unit_grid_y.flatten()))
    code_grid = np.column_stack((x_arr, y_arr))
    if N_codes < len(unit_grid):
        N_pad = len(unit_grid) - N_codes
        code_grid = pad_grid_array_with_zeros(code_grid, N_pad)
    affine_mat, inliers = cv2.estimateAffinePartial2D(unit_grid, code_grid)
    scale = np.sqrt(291.05714286**2 + 14.48571429**2)'''
...
def correct_barcode_position():
    ...
    for code in codes_1d:
        ...
        if barcode_caught_on_left and barcode_caught_on_top:
            ...
            barcode_suspected_top = code.rect.top - int(not barcode_caught_on_top)*barcode_max_height
            barcode_suspected_left = code.rect.left - int(not barcode_caught_on_left) * barcode_max_width
            #barcode_img = img[code.rect.top-10:code.rect.top+barcode_max_height+10, code.rect.left-10:code.rect.left+barcode_max_width+10]
            barcode_img = img[barcode_suspected_top - 10:barcode_suspected_top + barcode_max_height + 10, barcode_suspected_left - 10:barcode_suspected_left + barcode_max_width + 10]
            barcode_check = decode_1d(barcode_img)
            if len(barcode_check):
                code_x.append(barcode_suspected_left)
                code_y.append(barcode_suspected_top)
            else:
                print('fake barcode?')
                
                if barcode_caught_on_left:
                    code_x.append(code.rect[0])
                else:
                    code_x.append(code.rect[0] - barcode_max_width)
                if barcode_caught_on_top:
                    code_y.append(code.rect[1])
                else:
                    code_y.append(code.rect[1] - barcode_max_height)'''
        #plt.plot(code_x[-1], code_y[-1], '*b')
        #plt.plot(code.rect[0], code.rect[1], '*r')
        #plt.text(code.rect[0], code.rect[1], str(ii))
        #plt.text(code_x[-1], code_y[-1], str(ii))
        #ii += 1
    ...
    for img_file in img_files:
        ...
        if args.image_contains_individual_labels:
            ...
            if new_try:
                ...
                '''for ii in range(N_codes):
                    plt.text(ptsA_sorted[ii, 0], ptsA_sorted[ii, 1], str(ii), color='r')
                plt.plot(ptsA_sorted[:,0],ptsA_sorted[:, 1], '*r')
                for ii in range(N_codes):
                    plt.text(pts_B[ii, 0], pts_B[ii, 1], str(ii), color='b')
                plt.plot(pts_B[:,0],pts_B[:, 1], '*b')
                plt.imshow(img)'''
                ...
                '''label_x_shift = int(barcode_med_width/2)
                label_y_shift = int(barcode_med_height*2.5)
                label_height = int(barcode_med_height*7)
                label_width = int(barcode_med_width*1.75)'''
            ...
            '''else:
                # old version, with existing barcodes
                ax.imshow(image, aspect='auto')
                for code in codes_1d:
                    # text_image = img[code.rect.top-80:code.rect.top, code.rect.left-50:code.rect.left+120]
                    text_image = img[code.rect.top - 100:code.rect.top + 20, code.rect.left - 50:code.rect.left + 180]
                    save_temp_label_image()

                    rect = patches.Rectangle((code.rect.left - 50, code.rect.top - 100), 230, 120, linewidth=2,
                                             edgecolor='r',
                                             facecolor='none', alpha=0.6)
                    ax.add_patch(rect)
                    filenames.append(os.path.basename(img_file))
