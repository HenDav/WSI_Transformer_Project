...
dataset_id_tcga_dx = "TCGA_DX"
dataset_id_tcga_not_dx = "TCGA_not_DX"
...
dataset_containment_dict = {
    ...
    dataset_id_tcga: (dataset_id_tcga_dx,dataset_id_tcga_not_dx)
}
...
# for i in range(1, 21):
#     key = f"hormonal_label_noisy_{i}_column_name"
#     value = f"label_HL_noisy_{i}"
#     hormonal_label_column_names[key] = value
...
taylorX_numerical_columns = ["blindid", "osind", "inel", "StratMeno", "StratPlannedRT", "rxarm", \
         "RS", "age", "race", "ethnicity", "TumorSize", "Grade", "NucGrade", \
         "PrimSurg", "ChemRegGp", "TypeEndocrine", "ttfET", "InAnalysis",\
         "dfs", "dfsind", "drfi", "drfiind", "rfi", "rfiind", "survtime", "survstat",\
         "WithdrawConsent", "LostFU",  "typefdfs", "typefrec", "cause"]
taylorX_categorical_columns = []
taylorX_tabular_data_columns = taylorX_numerical_columns + taylorX_categorical_columns

def get_path_suffixes() -> Dict[str, Path]:
    path_suffixes = {
        dataset_id_tcga_dx: f"Breast/{dataset_id_tcga}",
        dataset_id_tcga_not_dx: f"Breast/{dataset_id_tcga}",
    ...
    while set_contains_dict_keys:
        for dataset_id in dataset_ids:
            if dataset_id in containment_set:
                ...
                datasets_folds_updated[dataset_id] = datasets_folds[dataset_id]
