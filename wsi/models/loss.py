...
### Taken from https://github.com/havakv/pycox/blob/master/pycox ###
...
def cox_ph_loss_sorted(log_h: Tensor, events: Tensor, eps: float = 1e-7) -> Tensor:
    ...
    where h = exp(log_h) are the hazards and R is the risk set, and d is event. Here 1 is noncensored.
    ...
    return - log_h.sub(log_cumsum_h).mul(events).sum().div(events.sum()).nan_to_num()
...
class CoxPHLoss(torch.nn.Module):
    ...
    limitation, but simple and fast.
    """
    def forward(self, log_h: Tensor, y: Tensor) -> Tensor:
        durations, events, _ = y.T
        return cox_ph_loss(log_h, durations, events)

class NaiveCensoredPinballLoss(torch.nn.Module):
    """
    Naive extension of the Pinball loss for censored data.

    We calculate $$p_\alpha^{IPC}(\tilde{y}, c, \hat{y})=\begin{cases}
			w(x) \alpha (\tilde{y}-\hat{y}), & \tilde{y} \geq \hat{y}\\
            w(x) (1-\alpha) (\hat{y}-\tilde{y}), & c > \tilde{y} \text{ and } \hat{y} > \tilde{y}\\
            0, & \text{otherwise}
        \end{cases},$$
    """
    ...
    def forward(self, log_h: Tensor, y: Tensor) -> Tensor:
        durations, events, _ = y.T
        pred_time = durations - log_h
        underestimated = pred_time > 0
        overestimated = torch.logical_and((pred_time < 0), events)
        return (self.alpha*(pred_time[underestimated]).sum()
                 +(1-self.alpha)*(-1.*pred_time[overestimated]).sum()
                )/(durations.shape[0])
